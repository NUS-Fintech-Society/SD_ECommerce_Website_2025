import { AdminRequest as AdminRequestData, useAuth } from "../providers/AuthProvider";
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!stringRegex.test(formData.name)) {
            setModalState({
                ...
                message: "Name must be up to 255 characters and only contain alphanumeric characters and spaces."
            });
            return;
        }
        ...
        if (!stringRegex.test(formData.role)) {
            setModalState({
                isOpen: true,
                title: "Validation Failed",
                message: "Role must be up to 255 characters and only contain alphanumeric characters and spaces."
            });
            return;
        }
        ...
        if (!mobileRegex.test(formData.mobileNumber)) {
            setModalState({
                isOpen: true,
                title: "Validation Failed",
                message: "Mobile Number must be between 8-15 digits and contain only numeric characters."
            });
            return;
        }
        ...
        if (!stringRegex.test(formData.organisation)) {
            setModalState({
                isOpen: true,
                title: "Validation Failed",
                message: "Organisation must be up to 255 characters and only contain alphanumeric characters and spaces."
            });
            return;
        }
        setIsLoading(true);
        ...
        <div>
            <div className="bg-blue-800">
                ...
                <div className="flex justify-between">
                    <h1 className="text-3xl text-white font-bold p-6">Request for Admin</h1>
                    <p className="bg-gray-300 rounded font-semibold m-6 px-8 py-2 flex items-center justify-center"> {user?.username}</p>
                </div>
                

                {/* Contact information section */}
                <form onSubmit={handleSubmit}>
                    <div className="mb-6 bg-white mx-10">
                        <div>
                            <h2 className="bg-blue-200 text-xl font-semibold mb-4 p-2">Your contact information:</h2>
                        </div>
                        ...
                        {/* Past activities */}
                        <div>
                            <div className="bg-blue-200 font-semibold p-2">
                            ...
                            <ul className="px-4">
                                {user?.adminRequests && user?.adminRequests?.length > 0 ? (
                                    ...
                                    <p className="py-4 flex justify-center items-center">No admin requests found.</p>
                                )}
                            </ul>
                            </div>
                        </div>
