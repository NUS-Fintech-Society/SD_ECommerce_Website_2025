...
export default function AdminRequests() {
  ...
  const handleReject = async (id: string) => {
    try {
        const response = await apiRequest("adminRequest", 'PUT', `reject/${id}`);
  ...
  const handleAccept = async (id: string) => {
    try {
        const response = await apiRequest("adminRequest", 'PUT', `accept/${id}`);
    ...
    <div className="bg-blue-800">
            ...
            <div className="bg-white rounded-lg m-4">
                ...
                <ul className="px-4">
                  {requests.length > 0 ? (
                    requests.map((req) => (
                      <li key={req._id} className="border-b py-4 flex justify-between">
                        <div>
